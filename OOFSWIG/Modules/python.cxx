...
/**********************************************************************
 * $Header: /users/langer/FE/CVSoof/OOF2/OOFSWIG/Modules/python.cxx,v 1.1 2014/06/25 18:40:20 lck Exp $
 *
 * python.cxx
 *
 * Python module.
...
void PYTHON::parse_args(int argc, char *argv[]) {
  ...
  for (i = 1; i < argc; i++) {	
	  ...
	  if(strcmp(argv[i],"-module") == 0) {
	    if (argv[i+1]) {
	      ...
	      strcpy(const_cast<char*>(module), argv[i+1]);
	      mark_arg(i);
	      mark_arg(i+1);
	      i+=1;
	    } else {
	    ...
	    if (argv[i+1]) {
	      ...
	      strcpy(const_cast<char*>(global_name), argv[i+1]);
	      mark_arg(i);
	      mark_arg(i+1);
	      i++;
	    } else {
  ...
  typemap_lang = const_cast<char*>("python");
...
void PYTHON::set_module(const char *mod_name, char **mod_list) {
  int i;
  ...
  strcpy(const_cast<char*>(module),mod_name);
...
void PYTHON::set_init(const char *iname) {
  set_module(iname,0);
}
...
void PYTHON::add_method(const char *name, char *function) {
...
void
PYTHON::get_pointer(const char *iname, const char *srcname, const char *src,const char *dest,
		    DataType *t, String &f, const char *ret)
{
...
// ----------------------------------------------------------------------
char *PYTHON::make_funcname_wrapper(const char *fnName)
{
...
void PYTHON::create_command(const char *cname,const char *iname) {
...
void PYTHON::create_function(char *name, const char *iname, DataType *d, ParmList *l)
{
  Parm    *p;
  int     pcount,i,j;
  ...
  f.add_local("PyObject *","_resultobj=0");
  ...
  while (p != 0) {
	...
	f.add_local("PyObject *",source,const_cast<char*>("0"));
	arglist << "&" << source;
	if (i >= (pcount-numopt))
    ...
    if ((tm = typemap_lookup("argout","python",p->t,p->name,target,const_cast<char*>("_resultobj")))) {
      outarg << tm << "\n";
...
void PYTHON::link_variable(char *name,const char *iname, DataType *t) {
...
void PYTHON::declare_const(const char *name,const char *, DataType *type,const char *value) {
...
char *PYTHON::usage_var(const char *iname, DataType *) {
...
char *PYTHON::usage_func(const char *iname, DataType *, ParmList *l) {
...
char *PYTHON::usage_const(const char *iname, DataType *,const char *value) {
...
void PYTHON::cpp_class_decl(char *name, char *rename,const char *type) {
    char temp[256];
    if (shadow) {
...
struct PyPragma {
  PyPragma(const char *method, char *text) : m_method(method), m_text(text), next(0) { }
...
void PYTHON::emitAddPragmas(String& output,const char* name,const char* spacing)
{
  PyPragma *p = pragmas;
  while (p) {
