...
generated by an old version of SWIG, which is included in OOFCanvas.
...
1. Unpack the tar file:  `tar -xf oofcanvas-x.y.z.tgz`

1. Go to the OOFCanvas directory: `cd oofcanvas-x.y.z`

1. Build OOFCanvas: `python setup.py build install --pythonAPI
   --magick`
   ...
   * If you want to build a debugging version, add `--debug` after
	 `build`.
